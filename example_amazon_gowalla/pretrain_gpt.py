...
from GPT_GNN.data import args_print
...
'''
    Dataset arguments
'''
parser.add_argument('--data_dir', type=str, default='./dataset/graph_reddit.pk',
                    help='The address of preprocessed graph.')
parser.add_argument('--pretrain_model_dir', type=str, default='./models/gpt_all_reddit',
...
graph_reddit: Graph = dill.load(open('./dataset/graph_pretrain_{}_{}.pk'.format(args.data, args.task_type), 'rb'))  # args.data_dir
...
for epoch in np.arange(args.n_epoch) + 1:
    ...
    for batch in np.arange(repeat_num) + 1:
        ...
        if valid_loss < best_val:
            best_val = valid_loss
            print('UPDATE!!!')
            torch.save(gpt_gnn.state_dict(), './models/gpt_pretrain_{}_{}.pk'.format(args.data, args.task_type))  # args.pretrain_model_dir
